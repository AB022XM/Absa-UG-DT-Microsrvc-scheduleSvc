...
import ug.co.absa.microsrvc.schedule.IntegrationTest;
...
class AbsaUgdtMicroscheduleSvcKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/absa-ugdt-microschedule-svc-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/absa-ugdt-microschedule-svc-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/absa-ugdt-microschedule-svc-kafka/unregister"));
                return;
            }
        }
